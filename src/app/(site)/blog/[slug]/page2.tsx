...
import Blog1 from '../../../../content/blog1.mdx'
...
export default async function Page({ params }) {
    ...
    const post: Post = await fetch(
      `${process.env.NEXT_PUBLIC_SERVER_URL}/api/posts?where[slug][equals]=${params.slug}&depth=2`,
      { next: { revalidate: 10 } },
    )
      .then(res => res.json())
      .then(res => res?.docs?.[0])
        ...
        <div className="hidden lg:absolute lg:inset-y-0 lg:block lg:h-full lg:w-full lg:[overflow-anchor:none]">
          ...
          <div className="grid grid-cols-1 justify-items-center homeAdjust">
            <div className="max-full text-center leading-relaxed">
              <h1 className="composedHeading">{post.title}</h1>
          ...
          >
          <img alt={`${post.title} featurede image`} src={post.featuredImage} width={1200} height={300} className="shadow-xl h-1/2 mx-auto" />
          ...
          {/* <Video videoEmbedCode={article ? article.articleVideoEmbed : null} /> */}
            ...
            <div className="py-8 flex flex-col md:flex-row">
              <div className="mx-auto w-full max-w-3xl">
                  <Blog1 />
                  {/* <RichText className={'article_text mt-8 pb-8 flex-col'} content={post.bodyContent1}  />    */}
                  <div className="header__underline"></div>     
              </div>
